...
void solve()
{
  ...
  while(condition)
  {
      condition=false;
      ans++;
      for(auto chapter : ready_read)
      {
          rem_chap.erase(chapter);
          for(auto nb : adj[chapter])
          {
              req[nb]--;
              if(req[nb]==0)
              {
                  if(nb>chapter)
                  ready_read.insert(nb);
                  else 
                  next_read.insert(nb);
              }
          }
      ...
      if(next_read.empty()==false)
      {
          condition=true;
          ready_read=next_read;
          next_read.clear();
      }
  }
  if(rem_chap.empty()==true)
  cout<<ans<<endl;
  else 
  cout<<-1<<endl;
