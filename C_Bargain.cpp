...
void solve()
{
  ...
  {
      ll x=s[i]-'0';
      dp1[i]=mod_add(dp1[i+1],mod_mul(power((ll)10,k),x));
      k++;
  }
  ll ans=0;
  for(i=n-1;i>=0;i--)
  {
       ll x=i+1;
       ans=mod_add(ans,mod_mul(dp1[i+1],x));
  ...
  {
      ll x=s[i]-'0';
      dp[i]=mod_add(mod_mul(dp[i-1],(ll)10),x);
      
  }
  for(i=1;i<n;i++)
  {
      ll val=mod_minus(power(10,n-i),1);
      val=mod_mul(val,mod_inv(9));
      ans=mod_add(ans,mod_mul(dp[i-1],val));
  }
  cout<<ans<<endl;
}
int main() {
