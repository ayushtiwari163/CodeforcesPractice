...
ll n,k;
ll rec(ll ind,ll pow,ll side,vector<vector<vll>> &dp)
{
    if(pow<1) return 0;
    if(ind>n || ind<1) return 1;
    if(dp[ind][pow][side]!=-1)
    return dp[ind][pow][side];
    if(side==0)
    {
        ll ans=((rec(ind-1,pow-1,1,dp)+rec(ind+1,pow,0,dp))%M)%M;
        dp[ind][pow][side]=ans;
        return ans;
    }
    else 
    {
        ll ans=((rec(ind+1,pow-1,0,dp)+rec(ind-1,pow,1,dp))%M)%M;
        dp[ind][pow][side]=ans;
        return ans;
    }
...
//        return;
//    }
   vector<vector<vll>> dp(n+1,vector<vll>(k+1,vll(2,-1)));
