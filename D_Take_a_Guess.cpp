// これを翻訳している間、あなたはあなたの人生のいくつかの貴重な瞬間を無駄にしました
//ジャイシュリーラム
#include <bits/stdc++.h>
#include <cmath>
#include <regex>
#define fi first
#define se second
#define pb push_back
#define mp make_pair
#define ba back
#define ppb pop_back
#define pqb   priority_queue<long long int>
#define eb emplace_back
#define eps 1e-6
#define vll vector<long long int>
#define pqs   priority_queue<int,vll,greater<long long int> >
#define sz(x) (int((x.size())))
#define all(x) (x).begin(),(x).end()
#define FAST ios_base :: sync_with_stdio (false); cin.tie (NULL)
using namespace std;
typedef long long int ll;
ll ivalue(string s)  {  ll x=0;  stringstream obj(s);  obj >> x;  return x;  } 
const ll M = 1e9 + 7;
const ll N = 1e5 + 5;
const ll inf = 2e18;
ll mod(ll x){   return (x%M);}
ll mod_minus(ll a, ll b){ ll ans= (mod(a)-mod(b)); if(ans<0) ans=mod(ans+M); return ans;}
ll mod_mul(ll a,ll b){  return mod(mod(a)*mod(b));}
ll mod_add(ll a,ll b){ return mod(mod(a)+mod(b));}
ll power(ll a,ll n){  if(n==0) return 1;  else if(n==1) return a;  ll R=power(a,n/2)%M;  if(n%2==0) {  return mod(mod_mul(R,R)); }   else { return mod(mod_mul(mod_mul(R,a),mod(R)));  }}
ll mod_div(ll a,ll b){// only if M is prime
ll ans=mod(a);
ll b1=power(b,M-2);
 ans= mod(mod_mul(ans,b1));
 return ans;
}
ll mod_inv(ll n){  return power(n,M-2);}
ll fact_mod(ll n){
vll fact(n+1);
fact[0]=1;
for(ll i=1;i<n+1;i++){
 fact[i]=mod_mul(fact[i-1],i);
}
return fact[n];
}
ll nCr_mod(ll n,  ll r)
{
if (r == 0 || n==0)
return 1;
ll fac[n + 1];
fac[0] = 1;
for (ll i = 1; i <= n; i++)
fac[i] = (fac[i - 1] * i) % M;
return (fac[n] * mod_inv(fac[r]) % M * mod_inv(fac[n - r]) % M) % M;
}
ll upper_fraction(ll a, ll b)
{
if(a%b==0)
return a/b;
else
return (a/b)+1;
}
bool isInt( double d )
{
double dummy;
return modf( d, &dummy ) == 0.0;
}
    
ll ask_and(ll x,ll y)
{
    cout<<"and "<<x<<' '<<y<<endl;
    ll ans;
    cin>>ans;
    return ans;
}  
ll ask_or(ll x,ll y)
{
    cout<<"or "<<x<<' '<<y<<endl;
    ll ans;
    cin>>ans;
    return ans;
}  
ll vector_to_num(vll a)
{
    ll i;
    ll ans=0;
    ll p=1;
   for(i=0;i<32;i++)
   {
      ans+=p*a[i];
      p=p*2;
   }
   return ans;
}
void func(vll &a,vll &b,ll p,ll q)
{
    for(ll x=0;x<32;x++)
     {
         if(q%2==0)
         {
             a[x]=0;
             b[x]=0;
         }
         else if(p%2==1)
         {
             a[x]=1;
             b[x]=1;
         }
         p=p/2;
         q=q/2;
     }
}
void solve()
{
  ll n,m;
  cin>>n>>m;
  vll vec;
  ll i=1,j=2,k=3;
   vll temp(32);
  while(k<=n)
  {
     ll i_a_b=ask_and(i,j);
     ll i_o_b=ask_or(i,j);
     vector<ll> a(32,-1);
     vector<ll> b(32,-1);
     vector<ll> c(32,-1);
     func(a,b,i_a_b,i_o_b);
     ll p=ask_and(i,k);
     ll q=ask_or(i,k);
     func(a,c,p,q);
     p=ask_and(j,k);
     q=ask_or(j,k);
     func(b,c,p,q);
    
     for(ll x=0;x<32;x++)
     {
         if(a[x]==-1)
         {
            if(b[x]!=-1)
            a[x]=b[x]^1;
            else if(c[x]!=-1)
            a[x]=c[x]^1;
         }
         if(b[x]==-1)
         {
             if(a[x]!=-1)
             b[x]=a[x]^1;
             else if(c[x]!=-1)
             b[x]=c[x]^1;
         }
         if(c[x]==-1)
         {
             if(a[x]!=-1)
             c[x]=a[x]^1;
             else if(b[x]!=-1)
             c[x]=b[x]^1;
         }

     }
     ll vla=vector_to_num(a);
     ll vlb=vector_to_num(b);
     ll vlc=vector_to_num(c);
    // cout<<vla<<' '<<vlb<<' '<<vlc<<endl;
     vec.pb(vla);
     vec.pb(vlb);
     vec.pb(vlc);
     k=k+3;
     i=i+3;
     j=j+3;
     temp=c;
  }
  while(vec.size()!=n)
  {
    ll x=vec.size();
    ll y=x+1;
    ll p=ask_and(x,y);
    ll q=ask_or(x,y);
    vll a(32,-1);
    for(i=0;i<32;i++)
    {
        if(q%2==0)
         {
             a[i]=0;
            
         }
         else if(p%2==1)
         {
             a[i]=1;
             
         }
         else 
         a[i]=temp[i]^1;
         p=p/2;
         q=q/2;

    }

      vec.pb(vector_to_num(a));
      temp=a;
  }
//   for(i=0;i<n;i++) cout<<vec[i]<<' ';
//   cout<<endl;
  sort(all(vec));
  cout<<"finish"<<' '<<vec[m-1]<<endl;
}
int main() {
FAST;
ll t;
t=1;
while(t--)
solve();
}