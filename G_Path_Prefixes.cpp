...
vll a, b, temp, ans;
void dfs(vector<vector<pair<ll, pair<ll, ll>>>> &adj, ll curr, ll par)
{
    for (auto nb : adj[curr])
    {
        if (nb.fi == par)
            continue;
        a[nb.fi] = a[curr] + nb.se.fi;
        b[nb.fi] = b[curr] + nb.se.se;
        temp.pb(b[nb.fi]);
        ll x = upper_bound(temp.begin(), temp.end(), a[nb.fi]) - temp.begin();
        ans[nb.fi] = x;
        dfs(adj, nb.fi, curr);
        temp.pop_back();
    }
